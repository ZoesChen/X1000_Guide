...
#define FOOTER_SIZE 6
    ...
    if (fread(footer, 1, FOOTER_SIZE, f) != FOOTER_SIZE) {
        printf("failed to read footer from %s (%s)\n", path, strerror(errno));
        fclose(f);
        return VERIFY_FAILURE;
    }
    ...
    printf("comment is %ld bytes; signature %ld bytes from end\n",(long)comment_size, (long)signature_start);

    if (signature_start - FOOTER_SIZE < RSANUMBYTES) {
        // "signature" block isn't big enough to contain an RSA block.
        printf("signature is too short\n");
        fclose(f);
        return VERIFY_FAILURE;
    }
...
#define EOCD_HEADER_SIZE 22
    ...
    if (fread(eocd, 1, eocd_size, f) != eocd_size) {
        printf("failed to read eocd from %s (%s)\n", path, strerror(errno));
        fclose(f);
        return VERIFY_FAILURE;
    }
...
#define BUFFER_SIZE 4096
    ...
    while (so_far < signed_len) {
        ...
        if (fread(buffer, 1, size, f) != size) {
            printf("failed to read data from %s (%s)\n", path, strerror(errno));
            fclose(f);
            return VERIFY_FAILURE;
        }
    ...
    for (i = 0; i < numKeys; ++i) {
        ...
        if (RSA_verify(pKeys+i, eocd + eocd_size - 6 - RSANUMBYTES,
                       RSANUMBYTES, sha1)) {
            printf("whole-file signature verified against key %ld\n", (long)i);
            free(eocd);
            return VERIFY_SUCCESS;
        } else {
		printf("failed to verify against key %ld\n", (long)i);
        }
    }
    free(eocd);
    printf("failed to verify whole-file signature\n");
    return VERIFY_FAILURE;
}
