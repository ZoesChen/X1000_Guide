...
static inline float32 float32_from_float16_xhs(int16_t a, int16_t b, flag ieee,
                                           float_status *status)
{
      float32 f_val;

      f_val = float16_to_float32_xhs((float16)a, (float16)b, ieee, status);
      f_val = float32_maybe_silence_nan(f_val);
...
#define XHS_FLOAT_BINOP(DEST, OP, ARG1, ARG2, ARG3, BITS)               \
    do {                                                                \
        float_status *status = &env->active_tc.msa_fp_status;           \
        int c;                                                          \
                                                                        \
        set_float_exception_flags(0, status);                           \
        DEST = float ## BITS ## _ ## OP(ARG1, ARG2, ARG3, status);      \
            c = update_msacsr(env, 0, IS_DENORMAL(DEST, BITS));         \
                                                                        \
            if (get_enabled_exceptions(env, c)) {                       \
                DEST = ((FLOAT_SNAN ## BITS >> 6) << 6) | c;            \
...
/* Ingenic AFU Architecture */
void helper_afu_fcmul_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
        uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t wtmp, *pwtmp = &wtmp;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
    wr_t *pws = &(env->active_fpu.fpr[ws].wr);
    wr_t *pwt = &(env->active_fpu.fpr[wt].wr);
    uint32_t i;
    ...
    case DF_WORD:
        for (i = 0; i < 4; i += 2) {
            MSA_FLOAT_BINOP(pwtmp->w[i], mul, pws->w[i], pwt->w[i], 32);
            MSA_FLOAT_BINOP(pwtmp->w[i+1], mul, pws->w[i+1], pwt->w[i+1], 32);
            MSA_FLOAT_BINOP(pwx->w[i], sub, pwtmp->w[i], pwtmp->w[i+1], 32);

            MSA_FLOAT_BINOP(pwtmp->w[i], mul, pws->w[i], pwt->w[i+1], 32);
            MSA_FLOAT_BINOP(pwtmp->w[i+1], mul, pws->w[i+1], pwt->w[i], 32);
            MSA_FLOAT_BINOP(pwx->w[i+1], add, pwtmp->w[i], pwtmp->w[i+1], 32);
        }
        break;
    default:
        assert(0);
...
void helper_afu_fexuplc_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
        uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
    ...
    case DF_WORD:
        for (i = 0; i < DF_ELEMENTS(DF_WORD); i++) {
            ...
            XHS_FLOAT_BINOP(pwx->w[i], from_float16_xhs, Lh(pws, i), Lh(pwt, i), ieee, 32);
        }
        break;
    default:
        assert(0);
...
void helper_afu_fexuprc_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
        uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
    ...
    case DF_WORD:
        for (i = 0; i < DF_ELEMENTS(DF_WORD); i++) {
            ...
            XHS_FLOAT_BINOP(pwx->w[i], from_float16_xhs, Rh(pws, i), Rh(pwt, i), ieee, 32);
        }
        break;
    default:
        assert(0);
...
void helper_afu_vshfr_df(CPUMIPSState *env, uint32_t wd,
                         uint32_t ws, uint32_t wt, uint32_t wr)
{
    ...
    wr_t *pws = &(env->active_fpu.fpr[ws].wr);
    wr_t *pwt = &(env->active_fpu.fpr[wt].wr);
    wr_t *pwr = &(env->active_fpu.fpr[wr].wr);
    uint32_t i;
    uint32_t k;

    for (i = 0; i < sizeof(wr_t); i++) {
        k = (pwr->b[i] & 0x3f) % (2 * (128/8));
        if ((pwr->b[i] & 0xc0) != 0)
            pwx->b[i] = 0;
        else {
            if (k < 16)
...
void helper_afu_bmr_df(CPUMIPSState *env, uint32_t wd,
                         uint32_t ws, uint32_t wt, uint32_t wr)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
    ...
    for (i = 0; i < 4; i++) {
        pwx->w[i] = (pws->w[i] & pwr->w[i]) | (pwt->w[i] & ~(pwr->w[i]));
    }

    msa_move_v(pwd, pwx);
}

void helper_afu_vsldi_b(CPUMIPSState *env, uint32_t wd,
                         uint32_t ws, uint32_t wt, uint32_t n)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
    wr_t *pws = &(env->active_fpu.fpr[ws].wr);
    ...
    for (i = 0; i < sizeof(wr_t); i++) {
        if ((i + n) < 16)
            ...
            pwx->b[i] = pwt->b[i+n-16];
    }

    msa_move_v(pwd, pwx);
}

void helper_afu_pcnt_v(CPUMIPSState *env, uint32_t df, uint32_t wd,
        uint32_t ws)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
...
void helper_afu_bext_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
        uint32_t ws)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
    ...
    case DF_BYTE:
        for (i = 0; i < 16; i++)
            pwx->b[i/8] |= ((pws->b[i] & ( 1 << (8 - 1))) >> 7) << (i%8);
        break;
    case DF_HALF:
        for (i = 0; i < 8; i++)
            pwx->h[i] |= ((pws->h[i] & ( 1 << (16 - 1))) >> 15) << i;
        break;
    case DF_WORD:
        for (i = 0; i < 4; i++)
            pwx->w[i] |= ((uint32_t)(pws->w[i] & ( 1 << (32 - 1))) >> 31) << i;
        break;
    default:
        assert(0);
    }
...
void helper_afu_bexp_0_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
        uint32_t ws)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
...
void helper_afu_bexp_1_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
        uint32_t ws)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
...
void helper_afu_accul_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
        uint32_t ws)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
...
void helper_afu_accur_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
        uint32_t ws)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
...
void helper_afu_accsl_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
        uint32_t ws)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
    ...
    case DF_HALF:
    {
        ...
        for (i = 0; i < 8; i++) {
            ...
            if ((a & 1 << 7) != 0)
                a = (0xff << 8) | a;
            pwx->h[i] = (int16_t)a + (int16_t)pwd->h[i];
        }
        break;
    }
    case DF_WORD:
    {
        ...
        for (i = 0; i < 4; i++) {
            ...
            if ((a & 1 << 15) != 0)
                a = (0xffff << 16) | a;
            pwx->w[i] = (int32_t)a + (int32_t)pwd->w[i];
        }
        break;
    }
    default:
...
void helper_afu_accsr_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
        uint32_t ws)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
...
void helper_afu_sll_v(CPUMIPSState *env, uint32_t wd,
                         uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t wtmp, *pwtmp = &wtmp;
    ...
    if (num == 0) {
        ...
        pwx->b[0] = pws->b[0] << num;
        for (i = 1; i < 16; i++) {
            pwtmp->b[i-1] = pws->b[i-1];
            pwx->b[i] = (pws->b[i] << num) | ((uint8_t)pwtmp->b[i-1] >> (8 - num));
        }
...
void helper_afu_slli_v(CPUMIPSState *env, uint32_t m,
                         uint32_t wd, uint32_t ws)
{
    wr_t wx, *pwx = &wx;
    wr_t wtmp, *pwtmp = &wtmp;
    ...
    if (m == 0) {
        ...
        for (i = 1; i < 16; i++) {
            pwtmp->b[i-1] = pws->b[i-1];
            pwx->b[i] = (pws->b[i] << m) | ((uint8_t)pwtmp->b[i-1] >> (8 - m));
        }
...
void helper_afu_bmu_v(CPUMIPSState *env, uint32_t wd,
                         uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
    ...
    for (i = 0; i < num + 1; i++) {
        if ((pwd->b[i/8] & (1 << i % 8)) != 0) {
            if((pws->b[j/8] & (1 << j % 8)) != 0)
                pwx->b[i/8] = pwx->b[i/8] | (1 << i % 8);
            else
                pwx->b[i/8] = pwx->b[i/8] & ~(1 << i % 8);
            j += 1;
        }
...
void helper_afu_srl_v(CPUMIPSState *env, uint32_t wd,
                         uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t wtmp, *pwtmp = &wtmp;
    ...
    if (num == 0) {
        ...
        for (i = 0; i < 15; i++) {
            pwtmp->b[i] = (uint8_t)pws->b[i] >> num;
            pwx->b[i] = (pws->b[i + 1] << (8 - num)) | pwtmp->b[i];
        }
        pwx->b[15] = (uint8_t)pws->b[15] >> num;
    }

    msa_move_v(pwd, pwx);
}

void helper_afu_srli_v(CPUMIPSState *env, uint32_t m,
                       uint32_t wd, uint32_t ws)
{
    wr_t wx, *pwx = &wx;
    wr_t wtmp, *pwtmp = &wtmp;
    ...
    if (m == 0) {
        ...
        for (i = 0; i < 15; i++) {
            pwtmp->b[i] = (uint8_t)pws->b[i] >> m;
            pwx->b[i] = (pws->b[i + 1] << (8 - m)) | pwtmp->b[i];
        }
        pwx->b[15] = (uint8_t)pws->b[15] >> m;
    }

    msa_move_v(pwd, pwx);
}

void helper_afu_movbt_b(CPUMIPSState *env, uint32_t wd,
                         uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
...
void helper_afu_movbp_b(CPUMIPSState *env, uint32_t wd,
                         uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
...
void helper_afu_sats_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
                         uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
    ...
    case DF_BYTE:
        for (i = 0; i < 16; i++) {
            if (i < 8) {
                if (((pwt->b[2*i + 1] & 0x80) == 0 ) && ((pwt->b[2*i + 1] & 0x7f) != 0))
                    pwx->b[i] = 0x7f;
                if (((pwt->b[2*i + 1] & 0x80) != 0 ) && ((pwt->b[2*i + 1] & 0x7f) != 0))
                    pwx->b[i] = 0x80;
                else
                ...
                if (((pws->b[2*(i - 8) + 1] & 0x80) == 0 ) && ((pws->b[2*(i - 8) + 1] & 0x7f) != 0))
                    pwx->b[i] = 0x7f;
                if (((pws->b[2*(i - 8) + 1] & 0x80) != 0 ) && ((pws->b[2*(i - 8) + 1] & 0x7f) != 0))
                    pwx->b[i] = 0x80;
                else
    ...
    case DF_HALF:
        for (i = 0; i < 8; i++) {
            if (i < 4) {
                if (((pwt->h[2*i + 1] & 0x8000) == 0 ) && ((pwt->h[2*i + 1] & 0x7fff) != 0))
                    pwx->h[i] = 0x7fff;
                if (((pwt->h[2*i + 1] & 0x8000) != 0 ) && ((pwt->h[2*i + 1] & 0x7fff) != 0))
                    pwx->h[i] = 0x8000;
                else
                ...
                if (((pws->h[2*(i - 4) + 1] & 0x8000) == 0 ) && ((pws->h[2*(i - 4) + 1] & 0x7fff) != 0))
                    pwx->h[i] = 0x7fff;
                if (((pws->h[2*(i - 4) + 1] & 0x8000) != 0 ) && ((pws->h[2*(i - 4) + 1] & 0x7fff) != 0))
                    pwx->h[i] = 0x8000;
                else
...
void helper_afu_satu_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
                         uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
    ...
    case DF_BYTE:
        for (i = 0; i < 16; i++) {
            if (i < 8) {
                if ((pwt->b[2*i + 1] & 0xff) != 0)
                    pwx->b[i] = 0xff;
                else
                    pwx->b[i] = pwt->b[2*i];
            } else {
                if ((pws->b[2*(i - 8) + 1] & 0xff) != 0)
                    pwx->b[i] = 0xff;
                else
                    pwx->b[i] = pws->b[2*(i - 8)];
            }
    ...
    case DF_HALF:
        for (i = 0; i < 8; i++) {
            if (i < 4) {
                if ((pwt->h[2*i + 1] & 0xffff) != 0)
                    pwx->h[i] = 0xffff;
                else
                    pwx->h[i] = pwt->h[2*i];
            } else {
                if ((pws->h[2*(i - 4) + 1] & 0xffff) != 0)
                    pwx->h[i] = 0xffff;
                else
                    pwx->h[i] = pws->h[2*(i - 4)];
            }
...
void helper_afu_addsl_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
                         uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
...
void helper_afu_addsr_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
                         uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
...
void helper_afu_addul_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
                         uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
...
void helper_afu_addur_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
                         uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
...
void helper_afu_subsl_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
                         uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
...
void helper_afu_subsr_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
                         uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
...
void helper_afu_subul_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
                         uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
...
void helper_afu_subur_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
                         uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
...
void helper_afu_mulsl_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
                         uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
...
void helper_afu_mulsr_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
                         uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
...
void helper_afu_mulul_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
                         uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
...
void helper_afu_mulur_df(CPUMIPSState *env, uint32_t df, uint32_t wd,
                         uint32_t ws, uint32_t wt)
{
    wr_t wx, *pwx = &wx;
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
...
void helper_afu_move_w(CPUMIPSState *env, uint32_t m, uint32_t n,
                         uint32_t wd, uint32_t ws)
{
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
    wr_t *pws = &(env->active_fpu.fpr[ws].wr);
