...
void helper_afu_ldins_w(CPUMIPSState *env, uint32_t wd, uint32_t n,
                         target_ulong addr)
{
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
    wr_t wx;
    ...
    for (i = 0; i < 4; i++)
        wx.w[i] = pwd->w[i];
    wx.w[n] = cpu_ldl_data(env, addr);
    memcpy(pwd, &wx, sizeof(wr_t));
}

void helper_afu_ldins_d(CPUMIPSState *env, uint32_t wd, uint32_t n,
                         target_ulong addr)
{
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
    wr_t wx;
    ...
    for (i = 0; i < 2; i++)
        wx.d[i] = pwd->d[i];
    wx.d[n] = cpu_ldq_data(env, addr);
    memcpy(pwd, &wx, sizeof(wr_t));
}

void helper_afu_stext_w(CPUMIPSState *env, uint32_t wd, uint32_t n,
                         target_ulong addr)
{
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
    int mmu_idx = cpu_mmu_index(env, false);
    ...
    ensure_writable_pages(env, addr, mmu_idx, GETRA());
    cpu_stl_data(env, addr, pwd->w[n]);
}

void helper_afu_stext_d(CPUMIPSState *env, uint32_t wd, uint32_t n,
                         target_ulong addr)
{
    wr_t *pwd = &(env->active_fpu.fpr[wd].wr);
    int mmu_idx = cpu_mmu_index(env, false);
