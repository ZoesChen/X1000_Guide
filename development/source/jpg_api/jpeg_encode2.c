...
VideoBuffer jpegbuf[BUFFER_COUNT];
...
void* jz_jpeg_encode_init(int width,int height)
{
	...
	jz_jpeg->yuyv_info.height = height;
        ...
        for (i = 0; i < reqbuf.count; i++)
        {
                ...
                // mmap buffer
                jpegbuf[i].length = buf.length;
                jpegbuf[i].start = (char *) mmap(0, buf.length, PROT_READ|PROT_WRITE, MAP_SHARED, fd, buf.m.offset);
                if (jpegbuf[i].start == MAP_FAILED) {
                ...
                printf("JPEG dst buffer %d: address=0x%x, length=%d\n", i, (unsigned int)jpegbuf[i].start, jpegbuf[i].length);
        }
...
/* 2: jpge strcut init */
	...
	if(jz_jpeg->src_buf_type != V4L2_MEMORY_MMAP)
        ...
        /* printf("length = %d\n", cap_buf.length); */
        /* printf("reserved  %d \n", cap_buf.reserved); */
        jz_jpeg->yuyv_info.BitStreamBuf = jpegbuf[cap_buf.index].start;
        jz_jpeg->bs_size = cap_buf.reserved;
#endif
