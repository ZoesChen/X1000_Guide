...
static void *earlysuspend_thread_func(void /*__unused*/ *arg)
{
    while (1) {
        if (wait_for_fb_sleep()) {
			LOGE("Failed reading wait_for_fb_sleep, exiting earlysuspend thread\n");
            return NULL;
        }
        ...
        if (wait_for_fb_wake()) {
			LOGE("Failed reading wait_for_fb_wake, exiting earlysuspend thread\n");
            return NULL;
        }
        pthread_mutex_lock(&earlysuspend_mutex);
        earlysuspend_state = EARLYSUSPEND_ON;
...
static int autosuspend_earlysuspend_enable(void)
{
    ...
    LOGV("autosuspend_earlysuspend_enable\n");
	...
	LOGV("autosuspend_earlysuspend_enable done\n");
...
static int autosuspend_earlysuspend_disable(void)
{
    ...
    LOGV("autosuspend_earlysuspend_disable\n");
    ...
    LOGV("autosuspend_earlysuspend_disable done\n");
...
void start_earlysuspend_thread(void)
{
	...
	LOGI("Starting early suspend unblocker thread\n");
...
struct autosuspend_ops *autosuspend_earlysuspend_init(void)
{
    ...
    if (ret < 0) {
        strerror_r(errno, buf, sizeof(buf));
        LOGW("Error writing 'on' to %s: %s\n", EARLYSUSPEND_SYS_POWER_STATE, buf);
        goto err_write;
    }
	LOGI("Selected early suspend\n");
