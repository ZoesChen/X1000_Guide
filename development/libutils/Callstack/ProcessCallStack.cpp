...
void ProcessCallStack::update() {
    ...
    while ((code = readdir_r(dp, &entry, &ep)) == 0 && ep != NULL) {
        ...
        ALOGW("%s: Got call stack for tid %d (size %zu) name:%s" ,
              __FUNCTION__, tid, threadInfo.callStack.size(),threadInfo.threadName.string());
    }
...
void ProcessCallStack::log(const char* logtag, log_priority priority,
