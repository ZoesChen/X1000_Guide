...
static int get_pagesize_from_nand(struct sfc_flash *flash)
{
	...
	u8 status;
	int ret = 0;
	ret = jz_sfc_nand_read(flash, 0, 0, buffer, 50, &status);
	if(ret)
		return ret;
...
struct jz_spi_support *jz_sfc_nand_read_id(struct sfc_flash *flash)
{
	...
	for (i = 0; i < number_spi_flash; i++) {
		...
		if ((params->id_manufactory == id_buf[0])&&(params->id_device == id_buf[1]))
			break;
	}
...
/*1. write to cache*/
#ifdef CONFIG_SPI_QUAD
	cmd[0].cmd = SPINAND_CMD_PRO_LOAD_X4;
#else
	cmd[0].cmd = SPINAND_CMD_PRO_LOAD;
#endif
	transfer[0].addr = column;
	transfer[0].addr_len = 2;
	transfer[0].addr_dummy_bits = 0;
	cmd[0].dataen = ENABLE;
	transfer[0].data = buffer;
	transfer[0].len = len;
	transfer[0].direction = GLB_TRAN_DIR_WRITE;
	transfer[0].data_dummy_bits = 0;
	transfer[0].cmd_info = &cmd[0];
	transfer[0].ops_mode = DMA_OPS;
#ifdef CONFIG_SPI_QUAD
	transfer[0].sfc_mode = TM_QI_QO_SPI;
#else
	transfer[0].sfc_mode = TM_STD_SPI;
#endif
	sfc_message_add_tail(&transfer[0], &message);
...
/*2. write enable*/
	cmd[1].cmd = SPINAND_CMD_WREN;
	transfer[1].addr = 0;
	transfer[1].addr_len = 0;
	transfer[1].addr_dummy_bits = 0;
	cmd[1].dataen = DISABLE;
	transfer[1].data_dummy_bits = 0;
	transfer[1].cmd_info = &cmd[1];
...
static int __init jz_sfc_probe(struct platform_device *pdev)
{
	...
	printk("-------------------end------------------------------------------\n");
#endif
	spi_flash = jz_sfc_nand_read_id(flash);
