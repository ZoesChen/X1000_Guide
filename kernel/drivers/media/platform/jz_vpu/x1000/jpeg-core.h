/* linux/drivers/media/platform/jz_vpu/x1000/jpeg-core.h
 *
...
#define X1000_JPEG_M2M_NAME		"jz-vpu"
...
/**
 * struct x1000_jpeg - JPEG IP abstraction
...
/**
 * x1000_jpeg_q_data - parameters of one queue
 * @fmt:	driver-specific format of this queue
 * @w:		image width
 * @h:		image height
 * @size:	image buffer size in bytes
 */
struct x1000_jpeg_q_data {
...
/**
 * x1000_jpeg_ctx - the device context data
...
struct x1000_jpeg_ctx {
	...
	struct x1000_jpeg_q_data	out_q;
	struct x1000_jpeg_q_data	cap_q;
	struct v4l2_fh		fh;
	bool			hdr_parsed;
...
/**
 * x1000_jpeg_buffer - description of memory containing input JPEG data
 * @size:	buffer size
 * @curr:	current position in the buffer
 * @data:	pointer to the data
 */
struct x1000_jpeg_buffer {
	unsigned long size;
	unsigned long curr;
	unsigned long data;
};
