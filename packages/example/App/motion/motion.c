...
static int motion_init(struct context *cnt)
{
	...
	jz_jpeg = jz_jpeg_encode_init(cnt->rotate_data.cap_width, cnt->rotate_data.cap_height);
//	vpu_start = (unsigned char *)JZMalloc(128, 1024*1024);
...
//		printf("frame_delay is %d, required_frame_time is %d elapsedtime is %d rolling_average is %d, required_frame_time is %d\n", frame_delay, required_frame_time, elapsedtime, rolling_average, required_frame_time);
        if (frame_delay > 0) {
