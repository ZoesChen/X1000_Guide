...
unsigned int vpu_phyaddr = 0;
unsigned int vpu_cxtan_addr = 0;
void * frame_cxtan = NULL;
#define CMD_VPU_GET_PADDR 111
...
//			len = s->buffers[s->buf.index].size;
//			frame_cxtan = (unsigned int)s->buffers[s->buf.index].ptr;
//			if (-1 == xioctl(vpu_fd, CMD_VPU_GET_PADDR, &vpu_phyaddr))
//			{
//				printf("Get phyaddr is fail\n");
//				return -1;
//			}
//			vpu_phyaddr = user_get_paddr((unsigned int)frame_cxtan);
//			memcpy(vpu_start, frame_cxtan, len);
//
			...
			len = s->buffers[s->buf.index].size;
			frame_cxtan = (void *)s->buffers[s->buf.index].ptr;
//			vpu_start = (unsigned char *)JZMalloc(128, len);
			printf("frame_cxtan is 0x%x, vpu_start is 0x%x\n\n", frame_cxtan, vpu_start);
			memcpy(vpu_start, frame_cxtan, len);
#endif
            return 0;
