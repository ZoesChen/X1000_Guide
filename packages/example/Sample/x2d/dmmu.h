...
static inline int dump_mem_info(struct dmmu_mem_info *mem, char * description)
{
	...
	printf("mem: %p, \t%s", mem, description?description:"");
	printf("\tvaddr= %p,", mem->vaddr);
	printf("\tsize= %d (0x%x)", mem->size, mem->size);
	printf("\tpaddr= 0x%08x", mem->paddr);
	printf("\tpage_count= %d", mem->page_count);
	printf("\tpages_phys_addr_table=%p", mem->pages_phys_addr_table);
	printf("\tstart_offset= %d", mem->start_offset);
	printf("\tend_offset= %d", mem->end_offset);
